<div>
    ...
    <script>
        ...
        const onDragStartForFormRow = function(e) {
            console.log("form-dragstart");
            //el.classList.add('dragging');

            dragFormStart = findTagParent(e.target, 'li');
            dragFormStart.classList.add('dragging');
        ...
        const onDragendForFormRow =  function(e) {
                e.preventDefault();
        ...
        const onDropForFormRow = function(e) {
                e.preventDefault();
                ...
                if(target) { // 대상 타켓이 선택된 경우
                    ...
                    if(dragFormStart != target) { // 자기 자신은 제외
                        ...
                        if(target.dataset['index'] === "999") {
                            ...
                            let insertedNode = pnode.insertBefore(dragFormStart,target);
                            insertedNode.dataset.tabIndex = tabid; // data 속성변경
                            console.log(dragFormStart);

                        } else {
                            // 서로 맞교환 이동
                            if(target.parentElement == dragFormStart.parentElement) {
                                console.log("맞교환");
                                targetNext = target.nextElementSibling;
                                srcNext = dragFormStart.nextElementSibling;
                                target.parentElement.insertBefore(dragFormStart, targetNext);
                                dragFormStart.parentElement.insertBefore(target, srcNext);
                            } else {
                                console.log("이동");
                                targetNext = target.nextElementSibling;
                                target.parentElement.insertBefore(dragFormStart, targetNext);
        ...
        const onDragoverForFormRow = function(e){
            e.preventDefault();
        };

        /*
        document.querySelectorAll('input[name="__tabbar"]').forEach(el=>{
            el.addEventListener('change',function(e){
                let tabIndex = e.target.dataset['index'];
                alert("tab changed = " + tabIndex);
                let items = el.parentElement.querySelectorAll('[data-tab-index="'+tabIndex+'"] .dragForms');
                console.log(items);
                items.forEach(li=>{
                    li.removeEventListener('dragstart', onDragStartForFormRow));
                    li.addEventListener('dragstart', onDragStartForFormRow);



                    li.removeEventListener('dragover', onDragoverForFormRow);
                    li.addEventListener('dragover', onDragoverForFormRow);



                    li.removeEventListener('drop', onDropForFormRow);
                    li.addEventListener('drop', onDropForFormRow);

                    li.removeEventListener('dragend',onDragendForFormRow);
                    li.addEventListener('dragend',onDragendForFormRow);
                })
        ...
        */
        ...
        dragForms.forEach(el => {
            el.addEventListener('dragstart', onDragStartForFormRow);

            el.addEventListener('dragenter', () => {
                //console.log("dragenter");
            });

            el.addEventListener('dragover', onDragoverForFormRow);

            el.addEventListener('dragleave', () => {
                //console.log("dragleave");
            });

            el.addEventListener('drop', onDropForFormRow);

            el.addEventListener('dragend',onDragendForFormRow);
        });
